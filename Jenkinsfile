...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="hyominlee/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/hyomsLee/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_hyominlee') {
                        oDockImage.push()
                    }
                }
            }
